...
To begin, we first activate the huskylens in I2C protocol to communicate with the microbit and place it in color detection mode. After that, we have a secuence to drive out the parking lot, where it uses the ultrasonic sensors to determine where is the closest wall and then proceeds to go forward and backwards until its completely out of the parking space. In addition, we start with the rear motor at a slightly higher power controlled by pwm to have inertia when starting.

We used 2 loops that run at the same time, one for the steering and one for the movement. In the movement loop, we run a code that simulates a pulse control that gives peaks of power while maintaining a low speed, we did this in order to not get stuck in the case it colides with an obstacle. In the other  loop, we place all the ultrasonic sensor variables and the huskylens sampling,. When it detects “ID1” corresponding to red, it turns to the right until it disappears from the field of view that we established by coordinates, the same with green “ID2” in the other direction. Finally, we put the same ultrasonic distance detection program that we used in the open challenge.

Para comenzar, en el bloque de inicio activamos el protocolo I2C de la huskylens para que se comunique con el microbit y se programe en modo de detección de color. Despues, tenemos la secuencia para salir del estacionamiento, donde utiliza los sensores ultrasonicas para determinar cual es la pared mas cercana y poder proceder a avanzar y retroceder hasta salir del estacionamiento. Adicionalmente, colocamos el motor trasero a una potencia mayor para tener mas inercia al iniciar.  

Usamos 2 bucles que se ejecutan simultaneamente, uno para la direccion y una para el movimiento. En el bucle del movimiento, ejecutamos un codigo que simula pulsos para tener picos de potencia mientras mantenemos una velocidad baja, asi podemos desatascarnos en caso de colisionar con un obstaculo o una pared. En el otro bucle, colocamos las variables de las distancias medidas por los sensores y el muestreo de la Huskylens, y colocamos las condicionales para la detección de colores de la Huskylens. Cuando la Camara detecta el ID1 correspondiente al color rojo, el vehiculo dobla hacia la derecha hasta que desaparece el ID de el campo de vision que establecemos con coordenadas, lo mismo pasa con el verde o ID2 en la otra direccion. Finalmente, despues de las condicionales de los obstaculos utilizamos el mismo codigo que en el desafio abierto.  
